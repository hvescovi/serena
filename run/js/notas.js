
//$("#myip").text("192.168.15.8");
//$("#myip").text("172.18.0.2");

// when the document is ready...
$(function () {

    // definições reusáveis neste código
    // *********************************

    function jmessage(tipo, mensagem) {

        // tenta usar biblioteca Swal  

        /*
        try {
            if (tipo == "ERRO") {
                Swal.fire({
                    icon: 'error',
                    title: 'Erro:',
                    text: mensagem
                });
            } else {
                Swal.fire({
                    position: 'top-end',
                    icon: 'success',
                    title: mensagem,
                    showConfirmButton: true,
                    timer: 2500
                });
            }
        } catch (error) {
          */
        // se der erro, mostra alert simples
        alert(mensagem);
        //}
    }

    var HTML_carregando = ' <img src="images/carregando.gif" id="carregando" height="30" class="d-none">';

    function manipular_carregando(show_hide) {
        if (show_hide == "show") {
            $("#carregando").removeClass("d-none");
        } else {
            $("#carregando").addClass("d-none");
        }
    }


    // início dos eventos
    // ******************

    //alert(document.URL);
    if (document.URL.startsWith("http://localhost")) {
        $("#myip").text("localhost");
    } else if (document.URL.startsWith("http://k8master")) {
        $("#myip").text("k8master.blumenau.ifc.edu.br");
    } else {
        url = document.URL;
        pos = url.search("/notas.html");
        if (pos > 0) {
            protocolo = "http://"
            http = protocolo.length;
            meuip = url.substring(http, pos);
            temdoisp = meuip.search(":");
            if (temdoisp > 0) {
                meuip = meuip.substring(0, temdoisp);
            }
            $("#myip").text(meuip);
        } else {
            alert("ERRO: não localizei URL");
        }
    }
    //$("#myip").text("k8master.blumenau.ifc.edu.br");

    // iniciar o círculo

    // qual elemento foi clicado
    //var eu = $(this).attr('id');
    // obtém o id da questão
    //alert(eu);








    myip = $("#myip").text();

    // obtém do backend qual é o círculo ativo
    url = 'http://' + myip + ':4999/circulo_ativo';

    $.ajax({
        url: url,
        method: 'GET',
        dataType: 'json',
        success: function (resultado) {
            if (resultado.message == "ok") {
                circ = resultado.details.id;
                //console.log("circulo ativo: " + circ);
                $("#circulo_id_digitado").val(circ);
                //carregar(circ)

            } else {
                jmessage("ERRO", "Não foi possível encontrar círculo ativo: " + resultado.details);
            }
        },
        error: function () {
            jmessage("ERRO", "Erro na leitura dos dados, verifique o backend");
        }
    });

    function carregar(circulo) {

        // vamos obter a informação dos respondentes
        url = 'http://' + myip + ':4999/gerar_nota_alunos/' + circulo;

        $.ajax({
            url: url,
            type: 'GET',
            dataType: 'json', // vou receber a resposta em json,
            //data: dados, // dados a enviar    //JSON.stringify({ "message": "ok" }), // dados a enviar
            //contentType: "application/json",
            success: function (resultado) {
                if (resultado.message == "ok") {
                    // percorre os resultados
                    for (var resp of resultado.details) {
                        var lin = `<tr>
                                <td>${resp.respondente}</td>
                                <td>${resp.nota}</td>
                                </tr>`;
                        $("#tabela").append(lin);
                    }

                }
                else {
                    jmessage("ERRO", resultado.details);
                }
            },
            error: function () {
                jmessage("ERRO", 'ocorreu algum erro na leitura dos dados, verifique o backend');
            }

        });

    }

    $(document).on("click", "#bt_atualizar", function () {
        var novo_circ = $("#circulo_id_digitado").val();

        url = 'http://' + myip + ':4999/circulo/' + novo_circ;

        $.ajax({
            url: url,
            type: 'GET',
            dataType: 'json', // vou receber a resposta em json,
            success: function (resultado) {
                if (resultado.message == "ok") {
                    $("#info_circulo").text(resultado.details.nome + " EM " + resultado.details.data);
                    $("#tabela").empty();
                    carregar(novo_circ);
                }
                else {
                    jmessage("círculo inválido!");
                }
            },
            error: function () {
                jmessage("ERRO", "Erro na leitura dos dados, verifique o backend");
            }
        });

    });


});